...
const themes = [
  { id: "blue", name: "Blue", primary: "oklch(0.55 0.18 240)", accent: "oklch(0.65 0.15 220)" },
  { id: "green", name: "Green", primary: "oklch(0.55 0.18 160)", accent: "oklch(0.65 0.15 140)" },
  { id: "purple", name: "Purple", primary: "oklch(0.55 0.18 280)", accent: "oklch(0.65 0.15 300)" },
  { id: "orange", name: "Orange", primary: "oklch(0.60 0.18 40)", accent: "oklch(0.70 0.15 60)" },
]
...
export function Settings({
  ...
  const t = {
    ...
    pairWithPartner: language === "en" ? "Pair with Partner" : "パートナーと繋がる",
    ...
    selectEvent: language === "en" ? "Select an event to edit" : "編集するイベントを選択してください",
    noEvents: language === "en" ? "No events to edit" : "編集するイベントがありません",
    ...
    confirmDelete: language === "en" ? "Are you sure you want to delete?" : "本当に削除しますか？",
    confirmDeleteDesc:
      language === "en"
  ...
  const handlePairSubmit = () => {
    if (pairPartnerName.trim() && pairPartnerCode.trim() && pairPartnerCode.length === 6) {
  ...
  return (
    <div className="space-y-6 pb-4 overflow-y-auto" style={{ maxHeight: "calc(100vh - 112px)" }}>
      <div>
        <h2 className="text-xl font-semibold mb-4 text-foreground">{t.accountInfo}</h2>
            ...
            {account.partnerId ? (
              <>
                <div className="flex justify-between items-center pt-2 border-t">
                  ...
                  <span className="font-semibold text-foreground">{account.partnerName}</span>
                </div>
                <div className="flex justify-between items-center">
              ...
              <div className="pt-2 border-t">
                {!showPairingForm ? (
                  <Button onClick={() => setShowPairingForm(true)} variant="outline" className="w-full" size="sm">
                    {t.pairWithPartner}
                  </Button>
                ) : (
                  <div className="space-y-3 pt-2">
                      ...
                      <Input
                        ...
                        value={
                          pairPartnerCode.length > 3
                            ? `${pairPartnerCode.slice(0, 3)}-${pairPartnerCode.slice(3)}`
                            : pairPartnerCode
                        }
                    ...
                    <div className="flex gap-2">
                      <Button
                        ...
                        disabled={!pairPartnerName.trim() || pairPartnerCode.length !== 6}
                        size="sm"
                        className="flex-1"
                      >
                        {t.pair}
      ...
      <div>
        <h2 className="text-xl font-semibold mb-4 text-foreground">{t.eventManagement}</h2>
        <Card className="p-4 bg-card border-border">
          {!showEditSection ? (
            <Button onClick={() => setShowEditSection(true)} variant="outline" className="w-full">
              <Edit className="w-4 h-4 mr-2" />
              {t.editEvents}
            </Button>
          ) : (
            <div className="space-y-3">
              <p className="text-sm text-muted-foreground mb-3">{t.selectEvent}</p>
              {events.length === 0 ? (
                ...
                <>
                    ...
                    <div key={event.id}>
                      {editingEvent?.id === event.id ? (
                        <Card className="p-4 bg-muted/50 border-primary">
                          <div className="space-y-3">
                            <div>
                              <label className="text-xs text-muted-foreground mb-1 block">{t.eventName}</label>
                              <Input
                                value={editTitle}
                            ...
                            <div className="flex gap-2">
                              <Button onClick={handleSaveEdit} disabled={!editTitle.trim()} className="flex-1">
                                {t.save}
                              </Button>
                  ...
                  <Button variant="ghost" onClick={() => setShowEditSection(false)} className="w-full mt-2">
                    {t.cancel}
                  </Button>
                </>
              )}
